...
def str2csv(filePath, s, letters):
    '''
    将字符串写入到本地csv文件中
    :param filePath: csv文件路径
    ...
    # 构建完整的字符串
        complete_string = letters + '\n'  # 如果希望每个字母占一行，可以添加换行符
        complete_string += s  # 添加原始内容
...
if __name__ == '__main__':
    readfilePath = r'output_data/author_tmp.csv'
    writefilePath1 = r'output_data/node.csv'
    writefilePath2 = r'output_data/edge.csv'
    # 读取csv文件获取作者信息并存储到列表中
    ...
    print(edge_str)
    # 将字符串写入到本地csv文件中
    str2csv(writefilePath1, node_str,"Id,Label,Weight")
    str2csv(writefilePath2, edge_str,"Source,Target,Weight")
    print(1231)